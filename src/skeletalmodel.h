...
public:
    ...
    void recursiveDrawDots(const Camera& camera, const Joint* joint);
    //helper function for drawing cylinders
    void recursiveDrawCylinders(const Camera& camera, const Joint* joint, const Joint* parent);
    ...
    void recursiveBindWorldToJointTransforms(Joint* joint, Matrix4f parentTransform);

    // 2.3.1. Implement this method to compute a per-joint transform from
    // world-space to joint space in the BIND POSE.
    void computeBindWorldToJointTransforms();
    
    void recursiveCurrentJointToWorldTransforms(Joint* joint, Matrix4f parentTransform);
